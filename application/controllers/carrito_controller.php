    ...
    public function realizar_venta()
{
    ...
    if ($cart = $this->cart->contents()):
        ...
        foreach ($cart as $item):
            ...
            // Descuenta el stock a través del procedimiento almacenado
            // sp_actualizar_stock_producto (valida producto activo y stock
            // suficiente del lado de la base de datos antes de actualizar).
            // Códigos de retorno: 0 = éxito, 1 = producto inactivo/no
            // encontrado, 2 = stock insuficiente.
            $resultado_stock = $this->producto_model->sp_actualizar_stock($item['id'], $item['qty']);

            if ($resultado_stock !== 0) {
                log_message(
                    'error',
                    'realizar_venta: sp_actualizar_stock_producto devolvió código ' .
                    $resultado_stock . ' para producto_id=' . $item['id'] .
                    ' (cantidad=' . $item['qty'] . ') en venta_id=' . $venta_id
                );
            }
